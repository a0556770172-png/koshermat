...
// ==========================================================
// הגדרת פרטי כניסה קבועים (מייל + סיסמה) - שימושי בעיקר למי שנכנס
// דרך קוד גישת אורח או גישת חירום ועדיין אין לו מייל/סיסמה קבועים.
// מתבצע דרך Edge Function עם מפתח שירות, כדי לעדכן את המייל מיידית
// בלי לשלוח קישור אימות למייל החדש (כי המשתמש הזה לא אמור להיתקל
// בשום שלב אימות נוסף גם כשהוא "משדרג" את החשבון שלו).
// ==========================================================
function wireCredentialsForm() {
  ...
  form.addEventListener("submit", async (e) => {
    e.preventDefault();
    const email = document.getElementById("credentials-email").value.trim();
    const password = document.getElementById("credentials-password").value;
    if (!email || password.length < 6) {
      toast("יש להזין מייל וסיסמה (לפחות 6 תווים)", "error");
      return;
    }
    ...
    try {
      ...
      if (!accessToken) {
        toast("יש להתחבר מחדש כדי לבצע פעולה זו", "error");
        btn.disabled = false;
        btn.textContent = "שמור פרטי כניסה";
        return;
      }

      const { data, error } = await withRetry(() =>
        sb.functions.invoke("set-guest-credentials", {
          body: { email, password },
      ...
      if (error || !data || data.error) {
        toast("שגיאה בשמירת פרטי הכניסה: " + ((data && data.error) || (error && error.message) || "שגיאה לא ידועה"), "error");
        btn.disabled = false;
        btn.textContent = "שמור פרטי כניסה";
        return;
      }

      toast("פרטי הכניסה נשמרו! מעתה אפשר להתחבר עם המייל והסיסמה האלה", "success");
      form.reset();
      btn.disabled = false;
      btn.textContent = "שמור פרטי כניסה";
    } catch (err) {
      toast(
        "אין תגובה מהשרת - בדוק את החיבור לאינטרנט ונסה שוב: " + (err && err.message ? err.message : String(err)),
        "error"
      );
      btn.disabled = false;
      btn.textContent = "שמור פרטי כניסה";
